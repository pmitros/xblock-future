...
def futureclass(config={}):
    ''' Modify the class c to support future proposed features. 
    '''
    def futureclass_helper(original_class):
        ...
        if hasattr(original_class, "grade") and isinstance(original_class.grade, Grade):
            original_class.has_score = True
            original_class.max_score = lambda x: x.grade.get_maxscore()

        return original_class
    return futureclass_helper
